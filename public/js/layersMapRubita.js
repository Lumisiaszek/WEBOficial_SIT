  ...
  fetch("http://localhost/getcapas/P/"+proyecto_bd)
    ...
    .then(registros => {
      ...
      registros.forEach(capa => {
        ...
        cadenaEscritorio += `
          <div class='item-capa'>
              <input type='checkbox' id='${capa.NoGeoserver}' ${capa.AcShape ? 'checked' : ''}>
        ...
        cadenaMobile += `
          <div class='item-capa'>
              <input type='checkbox' id='${capa.NoGeoserver}_m' ${capa.AcShape ? 'checked' : ''}>
              <label for='${capa.NoGeoserver}_m'>${capa.NoShape}</label>
          </div>
        `;

        if (capa.AcShape){ //si ya viene activada de la BD - mostrarla
